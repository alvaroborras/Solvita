...
function makePhaseDetail(phase, data) {
    ...
    switch (action.type) {
        case 'PHASE_START': {
            ...
            if (existing) {
                return {
                    ...state,
                    phases: state.phases.map((p) => p.key === action.phase ? { ...p, status: 'running', detail: '' } : p),
                };
            }
            const newPhase = {
                key: action.phase,
        ...
        case 'PHASE_DONE': {
            ...
            return {
                ...state,
                phases: state.phases.map((p) => p.key === action.phase ? { ...p, status: 'done', detail } : p),
            };
        }
        case 'FINAL':
