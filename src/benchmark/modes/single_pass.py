"""Single-pass LLM benchmark adapter (model-agnostic)."""
...
def _resolve_single_pass_model(config: Dict[str, Any]) -> str:
    """Resolve the model name for single-pass mode from config/models.yaml."""
    yaml_root = UnifiedLLMClient._load_yaml_root(config)
    llm_block = yaml_root.get("llm", {})
    roles = llm_block.get("roles", {})
...
def build_single_pass_config(config: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    single_pass_config["model"] = _resolve_single_pass_model(single_pass_config)
...
def _write_single_pass_artifacts(
    ...
    if not output_dir:
        return
    root = Path(output_dir) / "artifacts" / "single_pass"
...
def run_single_pass_case(
    ...
    llm_config = build_single_pass_config(config)
