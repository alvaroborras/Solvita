...
def resolve_problem_key(*, contest_id: int | None, index: str | None, url: str | None) -> tuple[int, str]:
    if contest_id is not None and index:
        return int(contest_id), index.strip().upper()
...
def _parse_time_limit_ms(value: str) -> int | None:
    match = re.search(r"(\d+(?:\.\d+)?)\s*(milliseconds?|ms|seconds?|secs?|s)\b", value.lower())
    if not match:
        return None
    amount = float(match.group(1))
    unit = match.group(2)
    return int(amount) if unit.startswith("m") else int(amount * 1000)


def _parse_memory_limit_mb(value: str) -> int | None:
    match = re.search(r"(\d+(?:\.\d+)?)\s*(kilobytes?|kb|megabytes?|mb|gigabytes?|gb|bytes?)\b", value.lower())
    if not match:
        return None
    amount = float(match.group(1))
    unit = match.group(2)
...
def parse_codeforces_problem_html(html: str, contest_id: int, index: str) -> dict[str, Any]:
    ...
    time_limit = _normalize_text(time_limit_node.get_text(" ", strip=True).replace("time limit per test", "", 1)) if time_limit_node is not None else ""
    memory_limit = _normalize_text(memory_limit_node.get_text(" ", strip=True).replace("memory limit per test", "", 1)) if memory_limit_node is not None else ""
    ...
    if sample_tests_root is not None:
        ...
        if len(inputs) != len(outputs):
            raise ValueError(f"Codeforces sample count mismatch: {len(inputs)} inputs, {len(outputs)} outputs")
        public_tests = [
            {
...
def build_problem_payload(parsed: dict[str, Any], *, contest_id: int, index: str, rating: int | None, tags: list[str] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "_metadata": {
            ...
            "name": parsed["title"],
            "difficulty": rating,
            "url": build_problem_url(contest_id, normalized_index),
...
def fetch_problem_html(contest_id: int, index: str) -> str:
    request = urllib.request.Request(
        build_problem_url(contest_id, index),
